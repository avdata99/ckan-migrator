...
def import_groups(old_db, new_db):
    """
    Importa SOLO grupos (is_organization = false).
    """
    ret = {
    ...
    log.info("Fetching groups from old database...")
    old_db.cursor.execute('SET search_path TO public')
    new_db.cursor.execute('SET search_path TO public')

    old_db.cursor.execute('SELECT * FROM public."group" WHERE is_organization = FALSE')
    ...
    log.info(f" - Importing Groups... ({len(rows)} registros)")
    for r in rows:
        ret['total_rows'] += 1
        g = _transform(r)
        ...
        if adjusted != g['name']:
            ret['warnings'].append(f"Group '{g['name']}' renamed to '{adjusted}'")
            g['name'] = adjusted
        ...
        try:
            ...
            ret['errors'].append(f"Group id={g['id']} name={g['name']}: {e}")
            ret['skipped_rows'] += 1
